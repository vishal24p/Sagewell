...
|---|---|---|---|
| (post-V1) `feat/m14-test-fixup` | Deprecated test patterns replaced with real-boundary + typed-DTO equivalents. Combined pytest 190 passed, 0 deprecation warnings. Legacy test files remain for one release cycle (D-093). | (none assigned) | 2026-06-27 |
...
| 2026-06-27 | M14 test-fixup on `feat/m14-test-fixup` (commit `b89c792`) — deprecated test patterns replaced with real-boundary + typed-DTO equivalents. (a) `tests/application/observability/test_logs.py` `_StubAuditRepo` did not call `is_allowed_reason_code(...)` — masked TECHNICAL_ISSUES Issue 01; replaced by `test_logs_real_repository.py` (9 tests) that round-trips all 17 V1 reason codes (including the six M10/M11 codes added to `_ALLOWED_REASON_CODES`) through the real `InMemoryAuditLogRepository`. (b) `tests/release_gate/test_m14_rbac_suite.py` synthesized DTOs with `type("Cmd", (), {...})()` — replaced by `test_m14_rbac_suite_typed.py` (7 tests) built via canonical `VerifyCitationsCommand` + `RetrieveAuthorizedCommand` factories. (c) `tests/application/evaluation/test_ragas.py` + `tests/application/llm_guard/test_llm_guard.py` shadowed `self.score` / `self.verdict` over method names — replaced by `test_ragas_real_class.py` (3) and `test_llm_guard_real_class.py` (5) with unambiguous `_RealisticScorer` / `_RealisticGuardrailModel`. (d) `pyproject.toml` `asyncio_default_fixture_loop_scope` pinned to `session` to silence `PytestDeprecationWarning`. Production fixes (minimal / scoped): `src/domain/ports/reason_codes.py` adds the six M10/M11 codes to `_ALLOWED_REASON_CODES`; `src/application/observability/logs.py` accepts optional `Clock` injection in `RecordRetrievalLog` + `RecordGuardVerdict`. Migration plan at `docs/HANDOFF/TEST_MIGRATION.md`; legacy tests retained for one release cycle pending D-093. Combined pytest 190 passed (was 166; net +24), 52 sandbox-skips, 0 failed; `-W error::DeprecationWarning` and `-W error::PendingDeprecationWarning` both clean. |
